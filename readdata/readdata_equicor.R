...
for (it in 1){
  for (i in 0:3){
    for (rule in para_rule){
      for (jobid in 1:nsim){
        tryCatch({
          oneres = read.csv(paste0("./debias_res_equicor/DebiasProg_equicor_cov_d", d, "_n", n, 
...
for (it in 1){
  for (i in 0:3){
    for (rule in c("1se", "mincv", "minfeas")){
      for (jobid in 1:nsim){
        tryCatch({
          oneres = read.csv(paste0("./debiascf_res_equicor/Debiascf_equicor_cov_d", d, "_n", n, 
                                   "_", jobid, "_x", i, "_rule", rule, "_intercept", it, ".csv"))
          oneres$x = i
...
for (jobid in 1:nsim){
  tryCatch({
    oneres = read.csv(paste0("./SIHR_res_equicor/SIHR_equicor_cov_d", d, "_n", n, "_", jobid, ".csv"))
...
for (jobid in 1:nsim){
  tryCatch({
    oneres = read.csv(paste0("./refit_res_equicor/refit_equicor_cov_d", d, "_n", n, "_", jobid, ".csv"))
