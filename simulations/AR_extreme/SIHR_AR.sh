...
#SBATCH --time=0-6:00:00                # wall clock limit (d-hh:mm:ss)
#SBATCH --mem-per-cpu=2500		          # memory per cpu
...
#SBATCH --job-name=SIHR_AR_ext          # Job name
#SBATCH -o Rout/SIHR_AR_%j.out         # output file (%j expands to jobID)
#SBATCH -e Rout/SIHR_AR_%j.err         # error log file (%j expands to jobID)
...
R CMD BATCH --no-save --no-restore "--args runInd=${SLURM_ARRAY_TASK_ID}" SIHR_AR.R "output/${SLURM_JOB_NAME}_${SLURM_ARRAY_TASK_ID}.Rout"