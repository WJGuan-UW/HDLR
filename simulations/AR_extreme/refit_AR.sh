...
#SBATCH --job-name=refit_AR_ext         # Job name
#SBATCH -o Rout/refit_AR_%j.out         # output file (%j expands to jobID)
#SBATCH -e Rout/refit_AR_%j.err         # error log file (%j expands to jobID)
...
R CMD BATCH --no-save --no-restore "--args runInd=${SLURM_ARRAY_TASK_ID}" refit_AR.R "output/${SLURM_JOB_NAME}_${SLURM_ARRAY_TASK_ID}.Rout"