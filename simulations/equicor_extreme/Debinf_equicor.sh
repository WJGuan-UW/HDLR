...
#SBATCH --time=4-00:00:00                # wall clock limit (d-hh:mm:ss)
...
#SBATCH --job-name=Deb_equicor_ext      # Job name
#SBATCH -o Rout/Deb_equicor_%j.out      # output file (%j expands to jobID)
#SBATCH -e Rout/Deb_equicor_%j.err      # error log file (%j expands to jobID)
...
R CMD BATCH --no-save --no-restore "--args runInd=${SLURM_ARRAY_TASK_ID}" Debinf_equicor.R "output/${SLURM_JOB_NAME}_${SLURM_ARRAY_TASK_ID}.Rout"