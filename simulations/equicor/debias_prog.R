...
HDLR_infer = function(X, Y, x, n_gamma=10, cv_rule='1se',
  ...
  m_deb = m_cur + sum(deb_res$w_obs * (Y - invlogit(X %*% theta_hat + alpha_hat))) / sqrt(n)
  sigmas = d.invlogit(X %*% theta_hat)
...
HDLR_cf = function(X, Y, x, n_gamma=10, cv_rule='1se',
  ...
  for (k in 1:2){
    ...
    m_deb = m_cur + sum(deb_res$w_obs * (Y[I2] - invlogit(X[I2,] %*% theta_hat + alpha_hat))) / sqrt(length(I2))
